...
class CommentRepository(BaseRepository[Comment]):
    ...
    async def get_by_post(
        ...
        stmt = select(self.model).where(self.model.post_id == post_id).offset(skip).limit(limit)
        result = await session.execute(stmt)
        return list(result.scalars().all())
    ...
    async def get_by_author(
        ...
        stmt = select(self.model).where(self.model.author_id == author_id).offset(skip).limit(limit)
        result = await session.execute(stmt)
        return list(result.scalars().all())