            ...
            with self._database.session() as session:
                ...
                if data.category_id is not None:
                    category = self._category_repo.get_by_id(
                        session=session, category_id=data.category_id
                    )
                    if not category:
                        error = DomainCategoryNotFoundException(
                ...
                if data.location_id is not None:
                    location = self._location_repo.get_by_id(
                        session=session, location_id=data.location_id
                    )
                    if not location:
                        error = DomainLocationNotFoundException(
                ...
                updated = self._repo.update(
                    session=session, post_id=post_id, **update_dict
                )
                return PostSchema.model_validate(updated, from_attributes=True)
        except (
            DomainPostNotFoundException,
            DomainCategoryNotFoundException,
            DomainLocationNotFoundException,
        ) as e:
            raise e
